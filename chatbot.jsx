  ...
  function Chatbot({ startOpen = false }) {
    ...
    function playStep(idx) {
      ...
      after(t, () => {
        ...
        else setReplies(step.replies.map((r, i) => ({ label: r, next: idx + 1, restart: r.startsWith('↻') })));
      });
    }
    ...
    function openChat() { setOpen(true); if (msgs.length === 0) reset(); }
    function collapse() { setOpen(false); }

    useEffect(() => { if (startOpen) reset(); return clearAll; }, []);
    useEffect(() => {
      ...
      <div style={{ height: '100%', display: 'flex', flexDirection: 'column', background: T.bg }}>
        ...
        <div style={{ paddingTop: 52, background: 'rgba(13,19,15,.92)', backdropFilter: 'blur(10px)', borderBottom: `1px solid ${T.line}`, flexShrink: 0 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 11, padding: '8px 16px 12px' }}>
            ...
            <div onClick={reset} title="Restart" style={{ cursor: 'pointer', width: 34, height: 34, borderRadius: 99, display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: `inset 0 0 0 1px ${T.line}` }}>
        ...
        {/* messages */}
        <div ref={scroller} style={{ flex: 1, minHeight: 0, overflowY: 'auto', padding: '16px 16px 8px', display: 'flex', flexDirection: 'column', gap: 9 }}>
        ...
        {/* quick replies + input */}
        <div style={{ flexShrink: 0, padding: '8px 14px 30px', background: 'rgba(13,19,15,.92)', backdropFilter: 'blur(10px)', borderTop: `1px solid ${T.line}` }}>
          {replies.length > 0 && (
